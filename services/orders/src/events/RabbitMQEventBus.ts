...
const RABBITMQ_ORDERS_QUEUE = process.env.RABBITMQ_ORDERS_QUEUE!;
...
export class RabbitMQEventBus implements EventBus {
  ...
  async connect() {
    ...
    try {
      ...
      this.channel.assertQueue(RABBITMQ_ORDERS_QUEUE, { durable: true });
      this.channel.bindQueue(
        RABBITMQ_ORDERS_QUEUE,
        RABBITMQ_FANOUT_EXCHANGE,
        '',
      );
      this.channel.consume(RABBITMQ_ORDERS_QUEUE, async (message) => {
        if (message) {
