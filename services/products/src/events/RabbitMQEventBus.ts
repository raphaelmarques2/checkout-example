...
const RABBITMQ_FANOUT_EXCHANGE =
  process.env.RABBITMQ_FANOUT_EXCHANGE || 'fanout-exchange';
const RABBITMQ_INVENTORY_QUEUE =
  process.env.RABBITMQ_INVENTORY_QUEUE || 'inventory-queue';
...
export class RabbitMQEventBus implements EventBus {
  ...
  async connect() {
    ...
    try {
      ...
      this.channel.assertQueue(RABBITMQ_INVENTORY_QUEUE, { durable: true });
      this.channel.bindQueue(
        RABBITMQ_INVENTORY_QUEUE,
        RABBITMQ_FANOUT_EXCHANGE,
        '',
      );
      this.channel.consume(RABBITMQ_INVENTORY_QUEUE, async (message) => {
        if (message) {
